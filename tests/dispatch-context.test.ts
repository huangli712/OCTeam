...
describe("dispatchToMember unit", () => {
    ...
    test("prepends <member-instruction> on FIRST dispatch then no-ops (promptDelivered wiring)", async () => {
        // Regression for the role-setup/task decoupling: member.prompt must be
        // delivered as <member-instruction> on the first real dispatch (NOT in
        ...
        // First dispatch: prompt is prepended as <member-instruction>
        ...
        expect(member.promptDelivered).toBe(true)

        // Second dispatch: prompt must NOT be re-prepended (delivered exactly once)
        await dispatchToMember(ctx, member, "[Your task]\nRun it again.", "/app")
        expect(captured).toHaveLength(2)
        expect(captured[1]).not.toContain("<member-instruction>")
    ...
    test("does not prepend when member has no prompt (legacy / pre-fix team graceful degradation)", async () => {
        ...
        expect(captured[0]).not.toContain("<member-instruction>")
    })
})
