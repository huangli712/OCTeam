...
/** Shared ctx builder that fills in boilerplate defaults. */
function makeCtxFor(
    root: string,
    opts?: {
        directory?: string;
        promptAsync?: (req: any) => any;
        create?: (req: any) => any;
        delete?: (req: any) => any;
    },
) {
    return makeCtx({
        ...
        overrides: {
            client: {
                app: { log: async () => ({}) },
                session: {
                    promptAsync: opts?.promptAsync ?? (async () => ({})),
                    messages: async () => ({ data: [] }),
                    create: opts?.create ?? (async () => ({ data: { id: "ses_default" } })),
...
describe("advanceToStage", () => {
    ...
    test("no activeTask: returns immediately without dispatching", async () => {
        ...
        const ctx = makeCtxFor(root, { promptAsync })
    ...
    test("member-instruction is prepended exactly once on first advanceToStage dispatch", async () => {
        ...
        expect(captured[0]).toContain("<member-instruction>")
...
describe("ensureMembersReady", () => {
    ...
    test("all members already have sessionId: no-op early return (no spawn)", async () => {
        ...
        const ctx = makeCtxFor(root, { create })
    ...
    test("prompt failure deletes the spawned session best-effort and preserves the original error", async () => {
        ...
        const ctx = makeCtxFor(root, { promptAsync, create, delete: deleteSession, directory: "/project" })
        await writeSpec(root, "alpha", sid, [{ name: "alice" }])
        const team = await makeTeam(root, sid, tracked, [makeMember("alice")])
    ...
    test("happy path: spawns session, sends role-setup prompt, flips member to running", async () => {
        ...
        const ctx = makeCtxFor(root, { promptAsync, create })
        ...
        // (delivered as <member-instruction>); role-setup is identity-only.
    ...
    test("worktree=true on a non-git project dir: createWorktree rejects with 'createWorktree(...) failed'", async () => {
        ...
        const ctx = makeCtxFor(root, { directory: root })
