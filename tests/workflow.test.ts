...
import { teamWorkflowTool, type WorkflowToolStep } from "../src/tools/workflow.js"
import type { PluginContext } from "../src/core/context.js"
import { makeMember, makeState, makeToolContext } from "./helpers.js"

type DispatchCall = { sessionId: string; text: string }

function makeCtx(outputs: Record<string, string>, calls: DispatchCall[] = [], storageRoot = "/tmp"): PluginContext {
    return {
        storageRoot,
        scope: "project",
        directory: "/app",
            ...
            branches: [
                {
                    id: "api",
                    steps: [
                        { kind: "task", member: "bob", task: "build api branch" },
                        { kind: "gate", verifier: "erin", criteria: "api branch passes" },
                    ],
                },
                {
                    id: "tests",
                    steps: [
                        { kind: "task", member: "carol", task: "build test branch" },
                        { kind: "gate", verifier: "eve", criteria: "test branch passes" },
                    ],
                },
            ],
        },
...
async function startToolLoweredFanoutWorkflow(maxErrored = 0): Promise<ToolLoweredFanoutFixture> {
    ...
    await initTeamState(root, makeState("alpha", sessions.master, [
    ...
    if (task?.type !== "workflow") throw new Error("Expected workflow task after team_workflow startup")

    return { calls, task, team, ctx, startupResult }
}

function makeFanoutHappyFixture(includeDownstream = true, maxErrored = 0): FanoutHappyFixture {
    const calls: DispatchCall[] = []
    const steps: WorkflowStep[] = [
        { kind: "task", member: "alice", task: "prepare shared context", completed: false },
        {
            kind: "fanout",
            completed: false,
            fanout: {
        ...
        },
        {
            ...
            completed: false,
            branch: { fanoutIndex: 1, branchId: "api", branchIndex: 0, joinIndex: 5 },
        },
        {
            kind: "task",
            member: "erin",
        ...
        },
        {
            ...
            completed: false,
            branch: { fanoutIndex: 1, branchId: "tests", branchIndex: 1, joinIndex: 5 },
        },
        {
            kind: "join",
            completed: false,
    ...
    if (includeDownstream) {
        steps.push({ kind: "task", member: "dave", task: "integrate branch results", completed: false })
    }
...
describe("handleWorkflowIdle (via processIdle): fanout frontier", () => {
    test("fanout happy path dispatches both branches and joins into downstream context", async () => {
        ...
        await processIdle(ctx, team, findTeamMember(team, "alice"), "ses_alice")
        ...
        await processIdle(ctx, team, findTeamMember(team, "carol"), "ses_carol")
    ...
    test("join fires once under repeated idle events after both branches finish", async () => {
        ...
        expect(calls.filter(call => call.sessionId === "ses_dave")).toHaveLength(1)
        expect(task.currentStageIndex).toBe(6)
        expect(task.activeStepIndices).toEqual([6])
    })
    ...
    test("fanout happy path completes after the downstream task idles", async () => {
        ...
        expect(team.activeTask).toBeDefined()
        expect(calls.filter(call => call.sessionId === "ses_lead" && call.text.includes("workflow_complete"))).toHaveLength(0)
        ...
        expect(team.status).toBe("idle")
        expect(team.activeTask).toBeUndefined()
        expect(calls.filter(call => call.sessionId === "ses_lead" && call.text.includes("workflow_complete"))).toHaveLength(1)
    })
    ...
    test("tool-lowered fanout task/gate branches join into downstream context", async () => {
        ...
        const { calls, task, team, ctx, startupResult } = await startToolLoweredFanoutWorkflow()

        expect(startupResult).toContain("team_workflow started")
        expect(task.steps?.map(step => step.kind)).toEqual(["task", "fanout", "task", "gate", "task", "gate", "join", "task"])
        ...
        await processIdle(ctx, team, findTeamMember(team, "alice"), sessionIdFor(team, "alice"))
        await processIdle(ctx, team, findTeamMember(team, "bob"), sessionIdFor(team, "bob"))
        await processIdle(ctx, team, findTeamMember(team, "carol"), sessionIdFor(team, "carol"))
        await processIdle(ctx, team, findTeamMember(team, "erin"), sessionIdFor(team, "erin"))
        expect(calls.some(call => call.text.includes("integrate branch results"))).toBe(false)
        await processIdle(ctx, team, findTeamMember(team, "eve"), sessionIdFor(team, "eve"))
        ...
        const daveCall = calls.find(call => call.text.includes("integrate branch results"))
        expect(daveCall).toBeDefined()
        expect(daveCall?.text).toContain("api branch output")
        expect(daveCall?.text).toContain("tests branch output")
        expect(task.activeStepIndices).toEqual([7])

        await processIdle(ctx, team, findTeamMember(team, "dave"), sessionIdFor(team, "dave"))
        expect(team.status).toBe("idle")
        expect(team.activeTask).toBeUndefined()
    })
    ...
    test("tool-lowered fanout branch gate failure within tolerance joins survivor output", async () => {
        ...
        const { calls, task, team, ctx } = await startToolLoweredFanoutWorkflow(1)
        await processIdle(ctx, team, findTeamMember(team, "alice"), sessionIdFor(team, "alice"))
        await processIdle(ctx, team, findTeamMember(team, "bob"), sessionIdFor(team, "bob"))
    ...
    test("tool-lowered fanout branch gate failure over tolerance fails the workflow", async () => {
        ...
        const { calls, task, team, ctx } = await startToolLoweredFanoutWorkflow()
        await processIdle(ctx, team, findTeamMember(team, "alice"), sessionIdFor(team, "alice"))
        await processIdle(ctx, team, findTeamMember(team, "bob"), sessionIdFor(team, "bob"))
        const erin = findTeamMember(team, "erin")
        erin.status = "errored"
        ...
        expect(team.activeTask).toBeUndefined()
        expect(calls.some(call => call.text.includes("integrate branch results"))).toBe(false)
        const leaderCall = calls.find(call => call.text.includes("workflow_failed:fanout:2:over_tolerance"))
        expect(leaderCall).toBeDefined()
        const runId = task.runId
        expect(runId).toBeDefined()
        const record = runId === undefined ? null : await readRunRecord(team.directory, runId)
    ...
    test("tool-lowered fanout branch INVALID retry uses that branch gate index", async () => {
        ...
        if (testsGate?.kind !== "gate") throw new Error("Expected tests branch gate")
        testsGate.onInvalid = "retry_verifier"
        testsGate.maxInvalidRetries = 1

        const invalidCtx = makeCtx({ [sessionIdFor(team, "eve")]: INVALID_VERDICT }, calls, ctx.storageRoot)
        await processIdle(invalidCtx, team, findTeamMember(team, "eve"), sessionIdFor(team, "eve"))

        expect(testsGate.invalidAttempts).toBe(1)
        const retryCall = calls.filter(call => call.sessionId === sessionIdFor(team, "eve")).at(-1)
        ...
        const retryEvent = events.find(event => event.kind === "retry" && event.member === "eve")
    ...
    test("fanout active branch actors complete their own step snapshots", async () => {
        ...
        await processIdle(makeCtx({ ses_alice: "api branch output" }, calls), team, findTeamMember(team, "alice"), "ses_alice")
        await processIdle(makeCtx({ ses_bob: "tests branch output" }, calls), team, findTeamMember(team, "bob"), "ses_bob")
        ...
        expect(task.steps?.[3]?.join?.joinedOutput).toContain("api branch output")
        expect(task.steps?.[3]?.join?.joinedOutput).toContain("tests branch output")
        expect(task.currentStageIndex).toBe(3)
        expect(team.status).toBe("idle")
        expect(team.activeTask).toBeUndefined()
        expect(calls.some(call => call.sessionId === "ses_lead" && call.text.includes("workflow_complete"))).toBe(true)
    })
    ...
    test("a fanout stray idle is rejected without capturing a branch snapshot", async () => {
        ...
        await processIdle(makeCtx({ ses_carol: "stray output" }, calls), team, findTeamMember(team, "carol"), "ses_carol")
    ...
    test("fanout partial failure within tolerance joins survivor output downstream", async () => {
        ...
        expect(joinStep?.join?.joinedOutput).toContain("tests branch output")
        expect(joinStep?.join?.joinedOutput).not.toContain("api branch output")
        expect(joinStep?.join?.joinedOutput).not.toContain("api packaged output")
        const daveCall = calls.find(call => call.sessionId === "ses_dave")
        expect(daveCall).toBeDefined()
        expect(daveCall?.text).toContain("integrate branch results")
        expect(daveCall?.text).toContain("tests branch output")
    ...
    test("fanout default over tolerance fails workflow with failed run status", async () => {
        ...
        expect(leaderCall?.text).toContain("workflow_failed:fanout:2:over_tolerance")
        const runId = task.runId
        expect(runId).toBeDefined()
        const record = runId === undefined ? null : await readRunRecord(team.directory, runId)
        expect(record?.status).toBe("failed")
    ...
    test("fanout all branches errored fails workflow even within numeric tolerance", async () => {
        ...
        expect(team.activeTask).toBeUndefined()
        const leaderCall = calls.find(call => call.sessionId === "ses_lead")
        expect(leaderCall?.text).toContain("workflow_failed:fanout:2:all_errored")
        const runId = task.runId
        expect(runId).toBeDefined()
        const record = runId === undefined ? null : await readRunRecord(team.directory, runId)
        expect(record?.status).toBe("failed")
    ...
    test("linear workflow member error fails immediately without fanout tolerance", async () => {
        ...
        const calls: DispatchCall[] = []
        const task = makeWorkflowTask({
            steps: [{ kind: "task", member: "alice", task: "linear work", completed: false }],
            currentStageIndex: 0,
        })
        const team = makeTeam({
            activeTask: task,
            members: [{ name: "alice", sessionId: "ses_alice", status: "errored", error: "provider outage" }],
        })
        const ctx = makeCtx({}, calls)
        ...
        expect(team.activeTask).toBeUndefined()
        const leaderCall = calls.find(call => call.sessionId === "ses_lead")
        expect(leaderCall?.text).toContain("member_error:alice:provider outage")
    })
})
