...
describe("resumeDispatch: workflow goto-after-restart", () => {
    test("a captured PASS with on_pass_goto jumps instead of advancing linearly", async () => {
        ...
        const task = makeTask({
            ...
            steps: [
                ...
                { kind: "gate", verifier: "bob", criteria: "ok", onPassGoto: 3, jumpCount: 0, completed: false },
                { kind: "task", member: "carol", task: "intermediate", completed: false },
                { kind: "task", member: "dave", task: "final", completed: false },
            ],
            currentStageIndex: 1,
    ...
    test("a captured FAIL with on_fail_goto jumps instead of failing the run", async () => {
        ...
        const task = makeTask({
            ...
            steps: [
                { kind: "task", member: "alice", task: "impl", completed: true, output: "impl" },
                { kind: "gate", verifier: "bob", criteria: "ok", onFail: "fail", onFailGoto: 2, jumpCount: 0, completed: false },
                { kind: "task", member: "carol", task: "recovery", completed: false },
            ],
            currentStageIndex: 1,
...
describe("resumeDispatch: workflow approval-pause-mid-restart", () => {
    ...
    test("approval_after pause survives restart: leader re-notified, next step NOT dispatched", async () => {
        ...
        const task = makeTask({
            type: "workflow",
            steps: [
                { kind: "task", member: "alice", task: "step 0", approvalAfter: true, completed: true, output: "done" },
...
describe("resumeDispatch: workflow retry/jump counter recovery", () => {
    test("a captured FAIL continues accumulating attempts (does NOT reset to 0)", async () => {
        ...
        const task = makeTask({
            ...
            steps: [
                { kind: "task", member: "alice", task: "impl", completed: true, output: "impl" },
                { kind: "gate", verifier: "bob", criteria: "ok", onFail: "retry", maxRetries: 2, attempts: 1, completed: false },
            ],
            currentStageIndex: 1,
            responses: { alice: "impl", bob: failVerdict },
        })
    ...
    test("a captured FAIL after retry exhaustion terminates (attempts already over cap)", async () => {
        ...
        const task = makeTask({
            ...
            steps: [
                { kind: "task", member: "alice", task: "impl", completed: true, output: "impl" },
                { kind: "gate", verifier: "bob", criteria: "ok", onFail: "retry", maxRetries: 2, attempts: 2, completed: false },
            ],
            currentStageIndex: 1,
            responses: { alice: "impl", bob: failVerdict },
        })
    ...
    test("a gate with a prior jumpCount continues counting (does NOT reset)", async () => {
        ...
        const task = makeTask({
            ...
            steps: [
                { kind: "task", member: "alice", task: "impl", completed: true, output: "impl" },
                { kind: "gate", verifier: "bob", criteria: "ok", onFail: "fail", onFailGoto: 0, jumpCount: 1, completed: false },
            ],
            currentStageIndex: 1,
            responses: { alice: "impl", bob: failVerdict },
        })
