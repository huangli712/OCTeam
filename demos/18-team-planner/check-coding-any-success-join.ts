/**
 * Check script: reverseString any-success-join fanout (Scenario 2).
 *
 * Reads ALL .md files from the run directory, extracts the LAST
 * ```typescript code block from each, transpiles and evaluates
 * reverseString, and asserts at least one implementation passes
 * all 4 test cases (any_success: first correct wins).
 *
 * Usage:  bun check-coding-any-success-join.ts <run_dir>
...
async function main(): Promise<void> {
    ...
    if (!runDir) {
        console.error("Usage: bun check-coding-any-success-join.ts <run_dir>");
        process.exit(2);
    }
    ...
    for (const [member, content] of mdFiles) {
        ...
        try {
            if (fn("hello") !== "olleh") { console.log(`  ${member}: reverseString('hello') !== 'olleh'`); continue; }
            if (fn("") !== "") { console.log(`  ${member}: reverseString('') !== ''`); continue; }
            if (fn("a") !== "a") { console.log(`  ${member}: reverseString('a') !== 'a'`); continue; }
            if (fn("ab") !== "ba") { console.log(`  ${member}: reverseString('ab') !== 'ba'`); continue; }
    ...
    if (!found) {
        fail("no member produced a working reverseString implementation");
    }

    console.log("PASS: at least one reverseString implementation is correct.");
}
