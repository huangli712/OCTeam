  ...
  "members": [
    {
      "name": "alice",
      "role": "simulator",
      "prompt": "ROOT TASK: Model a damped pendulum d2theta/dt2 + gamma*dtheta/dt + (g/L)*sin(theta) for small angles. Decompose into the undamped solution, the linear-damping perturbation, and the nonlinear correction, then aggregate.\n\nYou are the DECOMPOSER and the SOLE AGGREGATOR. Only YOU may aggregate the root task. Solver members (bob, carol) are FORBIDDEN from claiming the root — they are told to leave it for you.\n  STEP 0 (CRITICAL): Claim the root task FIRST (team_task_update status=\"claimed\" on the root). Until you claim a task, any <decompose> block you emit is IGNORED — the orchestrator only parses output from members holding a claimed/in_progress task.\n  STEP 1: Decompose the root into 3 subtasks (emit a <decompose> block — the orchestrator creates the subtasks automatically; do NOT call team_task_create): (a) undamped SHO solution theta0*cos(w*t) with w=sqrt(g/L), (b) linear-damping envelope exp(-(gamma/2)*t), (c) nonlinear sin-correction via one perturbation term (sin(theta) ~ theta - theta^3/6). After the orchestrator creates the subtasks, claim one (e.g. (a)) for yourself; let teammates claim the others. Coordinate via team_send_message.\n\nFor YOUR subtask (a): set g/L=1 (so w=1), gamma=0.2, theta0 small; give the undamped solution theta(t) = theta0*cos(t) and note its energy. End your subtask writeup with a one-line summary.\n\nHARD RULE — DO NOT GO IDLE BEFORE AGGREGATION (CONTRACT): After completing your own subtask, you MUST keep polling team_task_list every few seconds until all 3 subtasks show status=completed. Once the root becomes claimable again, claim it (team_task_update status=\"claimed\") and perform the aggregation. Going idle before emitting the final MODEL_VALID marker is a CONTRACT VIOLATION.\n\nAfter all 3 subtasks report back, AGGREGATE a piecewise model valid for gamma small and theta small: theta(t) ~ theta0*exp(-(gamma/2)*t)*cos(w_d*t) with the nonlinear correction noted. Confirm internal consistency.\n\nVERIFICATION SCOPE (HARD RULE): This is an ANALYTIC modeling task — verification is a sanity check, not the deliverable. ONE short inline script per subtask (plain RK4 at default tolerances over ~20 time units) plus at most ONE aggregated-model check at the root is enough. FORBIDDEN: multi-integrator cross-validation (e.g. DOP853/adaptive solvers at rtol<=1e-9), step-size convergence studies, brute-force parameter sweeps, and re-running verification after it passes once. Aggregate, emit the marker, idle.\n\nHARD RULE — FINAL MARKER (contract): Your final aggregated report MUST end with a line EXACTLY formatted (copy this verbatim, do not paraphrase, do not translate, place as the LAST line of your aggregation turn):\n<!-- MODEL_VALID: true -->\nThis marker is machine-extracted by the check script; a missing or altered marker = automatic FAIL. Do NOT go idle until this marker is emitted in the aggregation turn."
    },
    {
      "name": "bob",
      "role": "simulator",
      "prompt": "ROOT TASK context: recursive team modeling of a damped pendulum. You are a SOLVER member.\n\nHARD RULE — SCOPE (CONTRACT): You may ONLY claim subtask (b). You are FORBIDDEN from claiming the ROOT task — the root is reserved for alice (the decomposer/aggregator). If you see the root task become claimable, DO NOT claim it; leave it for alice. Claiming the root = CONTRACT VIOLATION.\n\nWatch the shared task list (team_task_list) for subtask (b): the LINEAR-DAMPING envelope of the underdamped small-angle oscillator theta'' + gamma*theta' + w0^2*theta = 0. When claimable, claim it (team_task_update) and solve: for gamma=0.2 and w0=1, the underdamped envelope is exp(-(gamma/2)*t); identify the decay constant gamma/2 = 0.1 and give the e-folding behavior.\n\nCoordinate with the decomposer via team_send_message.\n\nVERIFICATION SCOPE (HARD RULE): This is an ANALYTIC task — verification is a sanity check, not the deliverable. ONE short inline script (plain RK4 at default tolerances over ~20 time units) is enough. FORBIDDEN: multi-integrator cross-validation (e.g. DOP853/adaptive solvers at rtol<=1e-9), step-size convergence studies, brute-force parameter sweeps, and re-running verification after it passes once. Deliver the closed-form result and idle.\n\nFALLBACK (only if the root is unexpectedly assigned to you): You MUST perform the full aggregation (undamped SHO + linear damping + nonlinear correction, piecewise valid for gamma small and theta small) and your final report MUST end with a line EXACTLY formatted:\n<!-- MODEL_VALID: true -->\n\nYour normal report (subtask b) MUST end with a line exactly formatted: <!-- ENVELOPE_DECAY: 0.1 -->"
    },
    {
      "name": "carol",
      "role": "simulator",
      "prompt": "ROOT TASK context: recursive team modeling of a damped pendulum. You are a SOLVER member.\n\nHARD RULE — SCOPE (CONTRACT): You may ONLY claim subtask (c). You are FORBIDDEN from claiming the ROOT task — the root is reserved for alice (the decomposer/aggregator). If you see the root task become claimable, DO NOT claim it; leave it for alice. Claiming the root = CONTRACT VIOLATION.\n\nWatch the shared task list (team_task_list) for subtask (c): the NONLINEAR sin-correction. When claimable, claim it (team_task_update) and solve: expand sin(theta) = theta - theta^3/6 + ..., derive the leading (cubic) correction to the frequency (amplitude-dependent softening, domega ~ -theta0^2/16 for w0=1), and state the small-angle validity bound.\n\nCoordinate with the decomposer via team_send_message.\n\nVERIFICATION SCOPE (HARD RULE): This is an ANALYTIC task — verification is a sanity check, not the deliverable. ONE short inline script (plain RK4 at default tolerances over ~20 time units) is enough. FORBIDDEN: multi-integrator cross-validation (e.g. DOP853/adaptive solvers at rtol<=1e-9), step-size convergence studies, brute-force parameter sweeps, and re-running verification after it passes once. Deliver the closed-form result and idle.\n\nFALLBACK (only if the root is unexpectedly assigned to you): You MUST perform the full aggregation (undamped SHO + linear damping + nonlinear correction, piecewise valid for gamma small and theta small) and your final report MUST end with a line EXACTLY formatted:\n<!-- MODEL_VALID: true -->\n\nYour normal report (subtask c) ends with a one-line summary of the correction magnitude."
    }
  ]
}
```
...
- **Assertions**:
  ...
  2. APPROACH names collected from all members, deduplicated (lowercased; `_`→`-`; aliases folded: `generating`/`gf`→`generating-function`, `double-counting`/`bijective`→`combinatorial`), ≥2 distinct, and must include both `algebraic` and `combinatorial` (proving ≥2 independent paths succeeded)
  3. All LEMMA_HOLDS markers found are `true` (no leaf lemma falsified)
  4. Run terminal state: `<run_dir>/record.json` `status === "completed"` (no shallow pass on a failed/stalled run)
  5. Tree shape: root (depth 0) completed with ≥2 depth-1 subtasks (real decomposition happened; mailbox `*.jsonl` merged for markers)
  6. Participation: ≥3 distinct members produced ≥200 chars of output (no single-member direct-solve)
