...
const PIPELINE_LAST_STAGE = "erin.md";
...
async function main(): Promise<void> {
    ...
    // Read the last pipeline stage (erin.md) for pipeline markers
    let erinOutput: string;
    try {
        erinOutput = await readMember(runDir, PIPELINE_LAST_STAGE);
    } catch {
        // readMember already exits on error
        process.exit(2);
    }

    // Assertion 1: SPEC_OK: true
    if (!SPEC_OK_RE.test(erinOutput)) {
        fail("<!-- SPEC_OK: true --> marker not found in erin.md");
    }
    console.log("  pipeline marker: SPEC_OK=true present");

    // Assertion 2: IMPL: Stack
    if (!IMPL_STACK_RE.test(erinOutput)) {
        fail("<!-- IMPL: Stack --> marker not found in erin.md");
    }
    console.log("  pipeline marker: IMPL: Stack present");

    // Assertion 3: IMPL: Queue
    if (!IMPL_QUEUE_RE.test(erinOutput)) {
        fail("<!-- IMPL: Queue --> marker not found in erin.md");
    }
    console.log("  pipeline marker: IMPL: Queue present");

    // Assertion 4: PASS_COUNT: 4/4
    if (!PASS_COUNT_RE.test(erinOutput)) {
        fail("<!-- PASS_COUNT: 4/4 --> marker not found in erin.md");
    }
    console.log("  pipeline marker: PASS_COUNT=4/4 present");

    // Assertion 5: DOCS_OK: true
