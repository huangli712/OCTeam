...
- [ ] 4 个 check 脚本 `tsc -p scenarios/tsconfig.json` 通过（无类型错误）
...
```text
执行 scenarios/02-team-consensus/README.md「场景 1」的完整闭环并自动评判。
...
6. 运行：bun scenarios/02-team-consensus/check-math-sort-stability.ts <run_dir>
...
```text
执行 scenarios/02-team-consensus/README.md「场景 2」的完整闭环并自动评判。
...
5. 定位 <run_dir>
6. 运行：bun scenarios/02-team-consensus/check-physics-heat-diffusion.ts <run_dir>
7. 按退出码报告：0 = PASS，1 = FAIL，2 = 用法/IO 错误
...
```text
执行 scenarios/02-team-consensus/README.md「场景 3」的完整闭环并自动评判。
...
4. team_results 轮询至 master 收到汇总
5. 定位 <run_dir>
6. 运行：bun scenarios/02-team-consensus/check-coding-string-match.ts <run_dir>
7. 按退出码报告：0 = PASS，1 = FAIL，2 = 用法/IO 错误
...
```text
执行 scenarios/02-team-consensus/README.md「场景 4」的完整闭环并自动评判（挑战级，6 成员 × max_rounds=5，预计 ~35 min）。
...
6. 运行：bun scenarios/02-team-consensus/check-math-factoring-consensus.ts <run_dir>
7. 按退出码报告：0 = PASS，1 = FAIL，2 = 用法/IO 错误
