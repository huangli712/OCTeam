...
T+90m    master 收到汇总交付
T+90m    运行: bun scenarios/05-team-delegate/check-math-100-problems.ts <run_dir>
```
...
```text
执行 scenarios/05-team-delegate/README.md「场景 1」的完整闭环并自动评判。
...
6. 运行：bun scenarios/05-team-delegate/check-math-number-theory.ts <run_dir>
...
```text
执行 scenarios/05-team-delegate/README.md「场景 2」的完整闭环并自动评判。
...
5. 定位 <run_dir>
6. 运行：bun scenarios/05-team-delegate/check-physics-ode-suite.ts <run_dir>
7. 按退出码报告：0 = PASS，1 = FAIL，2 = 用法/IO 错误
...
```text
执行 scenarios/05-team-delegate/README.md「场景 3」的完整闭环并自动评判。
...
5. 定位 <run_dir>
6. 运行：bun scenarios/05-team-delegate/check-coding-cli-calc.ts <run_dir>
7. 按退出码报告：0 = PASS，1 = FAIL，2 = 用法/IO 错误
...
```text
执行 scenarios/05-team-delegate/README.md「场景 4」的完整闭环并自动评判。注意：此为挑战级场景，预计 ~90 min、8 成员并发。
...
6. 运行：bun scenarios/05-team-delegate/check-math-100-problems.ts <run_dir>
7. 按退出码报告：0 = PASS，1 = FAIL，2 = 用法/IO 错误
