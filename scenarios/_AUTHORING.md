...
OCTeam ships 9 orchestration primitives (`team_parallel`, `team_consensus`, `team_pipeline`, `team_loop`, `team_delegate`, `team_route`, `team_arbitrate`, `team_recurse`, `team_tollgate`). We are building a scenario catalog under `docs/orchestration-scenarios/`: each mode gets one directory containing a README (3 sub-scenarios: math / computational-physics / coding) plus 3 TypeScript check scripts.
...
- `docs/orchestration-scenarios/01-team-parallel/README.md`
- `docs/orchestration-scenarios/01-team-parallel/check-math-montecarlo-pi.ts`
- `docs/orchestration-scenarios/01-team-parallel/check-physics-harmonic-integrator.ts`
- `docs/orchestration-scenarios/01-team-parallel/check-coding-twosum.ts`

## Deliverables per mode (4 files)

```
docs/orchestration-scenarios/0N-team-<mode>/
├── README.md
├── check-math-<topic>.ts
...
1. `cd /home/yun/Working/devel/OCTeam && bunx tsc -p docs/orchestration-scenarios/tsconfig.json` — MUST exit 0 with no output.
2. All 4 files created at the correct paths under `docs/orchestration-scenarios/0N-team-<mode>/`.
