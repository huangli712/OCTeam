...
type FixWorkflowOp = "redispatch" | "skip" | "advance" | "fail"
...
async function applyFail(ctx: PluginContext, team: Team, reason: string | undefined): Promise<string> {
    ...
    await finishRun(ctx, team, `workflow_failed:${safeReason}`, "failed")
    await saveTeamState(team)
    return `team_fix_workflow failed workflow with reason workflow_failed:${safeReason}.`
}

async function applyWorkflowFix(ctx: PluginContext, team: Team, op: FixWorkflowOp, step: number | undefined, reason: string | undefined): Promise<string> {
    ...
    switch (op) {
        case "redispatch":
            return await applyRedispatch(ctx, team, target.task, step)
        case "skip":
            return await applySkip(ctx, team, target.task, step)
        case "advance":
...
export function teamFixWorkflowTool(ctx: PluginContext): ToolDefinition {
    return tool({
        ...
        args: {
            ...
            op: tool.schema.enum(["redispatch", "skip", "advance", "fail"]),
            step: tool.schema.number().int().min(1).optional().describe("1-based workflow step to repair. Defaults to the first active frontier step for redispatch/skip."),
        ...
        async execute(args, context) {
            ...
            await team.mutex.runExclusive(async () => {
                ...
                result = await applyWorkflowFix(ctx, team, args.op, args.step, args.reason)
