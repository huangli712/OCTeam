    ...
    return tool({
        ...
        async execute(args, context) {
            ...
            if (team.leadSessionId !== context.sessionID
                ...
                return "Error: team_remove_member is master-only "
                    + "(only the team's leader can remove members)"
            }
            ...
            await withLock(teamLifecycleLockPath(team.directory), async () =>
                ...
                try {
                    ...
                    logSwallowed(
                        ctx,
                        "team_remove_member: team state reload failed",
                        err,
                        { team: args.team_id },
                    )
                    stateError = true
                ...
                const currentIdx = team.members.findIndex(
                    member => member.name === args.member_name,
                )
                if (currentIdx === -1) {
                    memberMissing = true
                    return
                ...
                try {
                    ...
                    logSwallowed(
                        ctx,
                        "team_remove_member: team spec unreadable",
                        err,
                        { team: args.team_id },
                    )
                    specError = true
                ...
                try {
                    ...
                    try {
                        ...
                        logSwallowed(
                            ctx,
                            "remove: compensating spec revert failed after state save failure",
                            compensateErr,
                            { team: args.team_id },
                        )
                    }
