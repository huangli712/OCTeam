 ...
 *     enabled (approve → deliver idle: loop_complete:human_approved; reject
 *     at the cap → failed: loop_complete:human_rejected_max_rounds, reject
 *     below the cap → another round); without HITL → deliver (idle:
 *     loop_complete:decider_done)
...
export async function approveLoopDone(ctx: PluginContext, team: Team): Promise<void> {
    ...
    // The max_rounds HITL trigger in handleLoopIdle records this decision
    // before pausing (the decider-done path does not), so skip only when the
    // last entry is identical (same round + decision verb).
