 ...
 *
 * SECURITY: `kind` and `from` on REGULAR messages are taken verbatim from
 * the stored line (plain messages carry no authenticity — see mailbox.ts
 * "TRUST BOUNDARY" header). Directive priority is the exception: every
 * `kind:"directive"` line must pass the in-process authentication registry
 * (auth.ts) before it is honored. Only the legitimate write path
 * (writeMailboxMessage, called by team_send_message and team_intervene)
 * registers directives; the only legitimate directive source is
 * team_intervene, which writes `from: "master"`. A `kind:"directive"` line
 * that fails authentication — including one with any other `from` (a member
 * with FS write to .octeam/ impersonating control traffic) — is downgraded to
 * a regular message here.
 *
...
export function formatMailboxInjection(msgs: Message[], activeRunId?: string, teamName?: string): string {
    ...
    // Authentication: only directives whose (id, from, to, body, correlationId)
    // match a legitimate writeMailboxMessage registration AND whose runId (if
    // bound) matches the active run are honored. A forged line — whether
    // unregistered id, a replayed id with different content, or a cross-run
    // replay — is downgraded to a regular message (no [DIRECTIVE] prefix, no
    // priority).
