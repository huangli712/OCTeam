...
**成功标准（可机器评判）**：
- decomposer（`inc-exc`）输出含 `<!-- D4_FINAL: 9 -->`（三法聚合后的闭式数值）
...
**Role 选择理由**：`mathematician` 用 `build` agent，可推导、算数值、写证明——完全匹配本场景。`inc-exc` 兼任 decomposer（既是合法成员，又承担聚合）。
...
**参数选择**：
- `decomposer: inc-exc` — 必须是成员名，不能是 `master`；选 `inc-exc`（容斥法天然导出闭式，便于聚合）
...
T+0m    仅 dispatch decomposer (inc-exc)，附带递归契约
T+0~1m  inc-exc 分解 root → 创建 3 个 subtask（blockedBy DAG）
T+1m    recurrence / gen-func 被尾部的 re-prompt 唤醒，认领各自 subtask
T+1~7m  三成员并行推导：inc-exc=容斥, recurrence=递推, gen-func=母函数
T+7m    三叶 subtask 完成、回填结果 → 触发 root 聚合
T+8m    inc-exc 聚合三法 → 闭式 D_n 与 D_4=9，写 D4_FINAL
...
- **断言**：
  1. decomposer（`inc-exc.md`）存在且含 `D4_FINAL: 9`
...
**成功标准（可机器评判）**：
- decomposer（`sho`）输出含 `<!-- MODEL_VALID: true -->`（聚合模型在有效域内自洽）
...
**Role 选择理由**：`simulator` 专为数值/解析模拟设计，符合物理建模场景。`sho` 兼任 decomposer。
...
**参数选择**：
- `decomposer: sho` — 无阻尼 SHO 是聚合模型的主干（其余两项是它的扰动），适合做聚合锚点
...
```
T+0m    master 调用 team_recurse；root task 入共享任务列表（depth=0）
T+0m    仅 dispatch decomposer (sho)，附带递归契约
T+0~1m  sho 分解 root → 创建 3 个 subtask（blockedBy DAG）
T+1m    linear-damping / nonlinear-corr 被 re-prompt 唤醒，认领各自 subtask
T+1~7m  三成员并行建模：sho=无阻尼 SHO, linear-damping=包络, nonlinear-corr=修正
T+7m    三叶 subtask 完成、回填 → 触发 root 聚合
T+8m    sho 聚合分段模型，校验有效域，写 MODEL_VALID
...
- **断言**：
  1. decomposer（`sho.md`）含 `MODEL_VALID: true`
...
**成功标准（可机器评判）**：
- decomposer（`block-parser`）输出含 `<!-- CONVERTS: true -->`（聚合出可用 convert）
- 测试成员（`test-runner`）输出含 `<!-- PASS_COUNT: <n> -->`，n ≥ 5（覆盖 5 项特性且全部通过）
...
**Role 选择理由**：`coder` 用 `build` agent，专注实现、最小变更——贴合模块化构建。`block-parser` 兼任 decomposer（它拥有顶层 `convert` 的组装职责）。
...
**参数选择**：
- `decomposer: block-parser` — 块级解析是 convert 的入口，由其组装最终 convert 最自然
...
```
T+0m    master 调用 team_recurse；root task 入共享任务列表（depth=0）
T+0m    仅 dispatch decomposer (block-parser)，附带递归契约
T+0~1m  block-parser 分解 root → 创建 3 个 subtask（test-runner blockedBy 两个 parser）
T+1m    inline-parser 被 re-prompt 唤醒并认领 (b)；block-parser 自做 (a)
T+1~5m  block-parser 与 inline-parser 并行实现
T+5m    两 parser 完成 → test-runner 的 blockedBy 解除，被唤醒认领 (c)
T+5~8m  test-runner 组装 convert、跑测试套件、回填 PASS_COUNT
T+8m    block-parser 聚合最终 convert，写 CONVERTS
...
- **断言**：
  1. decomposer（`block-parser.md`）含 `CONVERTS: true`
  2. `test-runner.md` 含 `PASS_COUNT: <n>`，n ≥ 5（覆盖 5 项特性且全过）
...
4. team_results 轮询至 master 收到汇总（子任务：block-parser / inline-parser / tests）
5. 定位 <run_dir>
