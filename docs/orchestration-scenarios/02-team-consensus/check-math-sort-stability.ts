/**
 * Check script: small-sort stability consensus (3-way debate).
 *
 * Verifies the team_consensus run reached genuine agreement on a stable-sort
 * algorithm for n<50 nearly-sorted input: every member's final-round
 * <consensus> tag has agreed=true, names a recognized algorithm
 * (insertion|timsort|merge), and all three converged on the SAME choice.
 *
...
const KNOWN_RE = /^(insertion|timsort|merge)/i;
...
// Normalize a choice to its known-name prefix so "insertion sort",
// "Insertion-Sort", "timsort" all collapse to "insertion" / "timsort".
...
async function main(): Promise<void> {
    ...
    if (!runDir) {
        console.error("Usage: bun check-math-sort-stability.ts <run_dir>");
        process.exit(2);
    }
    ...
    for (const r of results) {
        ...
        if (!KNOWN_RE.test(choice!)) {
            fail(`member "${r.member}" choice "${choice}" does not match known names (insertion|timsort|merge)`);
        }
